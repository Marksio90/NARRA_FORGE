...
async def _semantic_analyze_title_with_ai(title: str, genre: str) -> dict:
    ...
    # Load prompt template from external file (450+ lines externalized for maintainability)
    _prompt_template_path = Path(__file__).parent.parent / "prompts" / "title_analysis_prompt.txt"
    prompt_template = _prompt_template_path.read_text(encoding="utf-8")
    prompt = prompt_template.format(title=title, genre=genre)
...
def _analyze_title(title: str, genre: str) -> dict:
    ...
    affectionate_terms = {
        ...
        "ukochana": {"role": "ukochana", "gender": "female", "theme": "romans"},
        "kochanek": {"role": "kochanek", "gender": "male", "theme": "romans"},
        "kochanka": {"role": "kochanka", "gender": "female", "theme": "romans"},
